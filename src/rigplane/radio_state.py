...
Canonical location: rigplane.core.radio_state
...
makes ``rigplane.radio_state`` literally the same module object as
``rigplane.core.radio_state``. This preserves attribute walks (incl.
stdlib names not in ``__all__``) and monkeypatch targets such as
``unittest.mock.patch('rigplane.radio_state.…', …)``.

The two import lines below are BOTH load-bearing — do not remove
either:

* ``from rigplane.core.radio_state import *`` — static-analysis
  ...
  ``from rigplane.radio_state import X`` triggers ``attr-defined``
...
* ``sys.modules[__name__] = _canonical`` — the runtime invariant.
  Makes ``rigplane.radio_state`` and ``rigplane.core.radio_state``
...
from rigplane.core.radio_state import *  # noqa: F401, F403
import rigplane.core.radio_state as _canonical
