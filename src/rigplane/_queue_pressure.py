...
Canonical location: rigplane.core._queue_pressure
...
makes ``rigplane._queue_pressure`` literally the same module object
as ``rigplane.core._queue_pressure``. This preserves attribute walks
(incl. stdlib names not in ``__all__``) and monkeypatch targets such
as ``unittest.mock.patch('rigplane._queue_pressure.…', …)``.

The two import lines below are BOTH load-bearing — do not remove
either:

* ``from rigplane.core._queue_pressure import *`` — static-analysis
  ...
  ``from rigplane._queue_pressure import X`` triggers ``attr-defined``
...
* ``sys.modules[__name__] = _canonical`` — the runtime invariant.
  Makes ``rigplane._queue_pressure`` and
  ``rigplane.core._queue_pressure`` the same module object so
...
from rigplane.core._queue_pressure import *  # noqa: F401, F403
import rigplane.core._queue_pressure as _canonical
