...
Canonical location: rigplane.runtime.radio
...
makes ``rigplane.radio`` literally the same module object as
``rigplane.runtime.radio``. This preserves attribute walks (incl.
stdlib names not in ``__all__``) and monkeypatch targets.

The two import lines below are BOTH load-bearing — do not remove
either:

* ``from rigplane.runtime.radio import *`` — static-analysis adapter.
* ``sys.modules[__name__] = _canonical`` — the runtime invariant.

If a future external consumer needs to reach a private (``_``-prefixed)
symbol through this shim, add an explicit ``from rigplane.runtime.radio
...
from rigplane.runtime.radio import *  # noqa: F401, F403
import rigplane.runtime.radio as _canonical
