...
Canonical location: rigplane.core.auth
...
makes ``rigplane.auth`` literally the same module object as
``rigplane.core.auth``. This preserves attribute walks (incl.
...
* ``sys.modules[__name__] = _canonical`` — the runtime invariant.
  Makes ``rigplane.auth`` and ``rigplane.core.auth`` the
...
from rigplane.core.auth import *  # noqa: F401, F403
import rigplane.core.auth as _canonical
