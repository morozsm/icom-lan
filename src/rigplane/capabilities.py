...
Canonical location: rigplane.core.capabilities
...
makes ``rigplane.capabilities`` literally the same module object as
``rigplane.core.capabilities``. This preserves attribute walks (incl.
...
* ``from rigplane.core.capabilities import *`` — static-analysis adapter.
  Mypy and ruff resolve re-exported names through star-imports; they
  do not model the ``sys.modules`` mutation. Without this line,
  every consumer of ``from rigplane.capabilities import X`` triggers
...
* ``sys.modules[__name__] = _canonical`` — the runtime invariant.
  Makes ``rigplane.capabilities`` and ``rigplane.core.capabilities`` the
...
from rigplane.core.capabilities import *  # noqa: F401, F403
import rigplane.core.capabilities as _canonical
