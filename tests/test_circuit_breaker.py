...
def test_circuit_opens_logs_warning(caplog: pytest.LogCaptureFixture) -> None:
    ...
    with caplog.at_level(logging.WARNING, logger="rigplane.rigctld.circuit_breaker"):
        cb.record_failure()
        cb.record_failure()
...
def test_open_transitions_to_half_open_after_timeout() -> None:
    ...
    with patch(
        "rigplane.rigctld.circuit_breaker.time.monotonic", return_value=_FAR_FUTURE
    ):
...
def test_open_to_half_open_logs_info(caplog: pytest.LogCaptureFixture) -> None:
    ...
    cb = CircuitBreaker(failure_threshold=1, recovery_timeout=5.0)
    cb.record_failure()
    with caplog.at_level(logging.INFO, logger="rigplane.rigctld.circuit_breaker"):
        with patch(
            "rigplane.rigctld.circuit_breaker.time.monotonic", return_value=_FAR_FUTURE
        ):
